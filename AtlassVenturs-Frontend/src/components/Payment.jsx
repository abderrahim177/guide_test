...
export default function CheckoutPage() {
  ...
  // معلومات تجريبية للحجز (يمكنك ربطها بـ props أو state عامة)
  const bookingDetails = {
    title: "High Atlas Trekking & Guide Experience",
    duration: "3 Days / 2 Nights",
    location: "Ait Bougmez, Azilal",
  ...
  const totalAmount = bookingDetails.gearTotal + bookingDetails.guideFee + bookingDetails.insuranceFee;
  ...
  if (isSubmitted) {
    return (
      <div className="max-w-md mx-auto mt-12 bg-white border border-emerald-200 rounded-3xl p-8 text-center space-y-4 shadow-lg font-['Poppins',sans-serif]">
        ...
        <h2 className="text-2xl font-extrabold text-stone-900">Payment Successful!</h2>
        <p className="text-xs text-stone-500">
          Your booking and equipment rental have been confirmed. We sent confirmation details to your WhatsApp and email.
        </p>
        <button 
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 font-['Poppins',sans-serif] text-stone-800">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div 
                ...
                className={`p-4 rounded-2xl border cursor-pointer flex items-center gap-3 transition-all ${
                  paymentMethod === "card" ? 'border-emerald-600 bg-emerald-50/40 shadow-xs' : 'border-stone-200 bg-white'
                }`}
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${paymentMethod === "card" ? 'bg-emerald-600 text-white' : 'bg-stone-100 text-stone-500'}`}>
                  <CreditCard className="w-5 h-5" />
                </div>
                <div>
                  <span className="font-bold text-xs text-stone-900 block">Credit / Debit Card</span>
                  <span className="text-[10px] text-stone-500">Instant international payment</span>
                </div>
              </div>
              ...
              <div 
                ...
                className={`p-4 rounded-2xl border cursor-pointer flex items-center gap-3 transition-all ${
                  paymentMethod === "deposit" ? 'border-emerald-600 bg-emerald-50/40 shadow-xs' : 'border-stone-200 bg-white'
                }`}
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${paymentMethod === "deposit" ? 'bg-emerald-600 text-white' : 'bg-stone-100 text-stone-500'}`}>
                  <Wallet className="w-5 h-5" />
                </div>
                <div>
                  <span className="font-bold text-xs text-stone-900 block">Pay on Arrival / Deposit</span>
                  <span className="text-[10px] text-stone-500">Pay cash directly to your guide</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {paymentMethod === "card" && (
            <div className="bg-white border border-stone-200 p-5 rounded-3xl space-y-4 shadow-2xs">
              <div className="flex items-center justify-between border-b border-stone-100 pb-3">
                <span className="font-bold text-xs text-stone-900">Card Information</span>
                <Lock className="w-4 h-4 text-emerald-600" />
              </div>

              <div className="space-y-3 text-xs">
                <div>
                  <label className="block font-medium text-stone-600 mb-1">Cardholder Name</label>
                  <input 
                    type="text" 
                    required 
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block font-medium text-stone-600 mb-1">Expiry Date</label>
                    <input 
                      type="text" 
                      required 
                      placeholder="MM/YY" 
                  ...
                  <div>
                    <label className="block font-medium text-stone-600 mb-1">CVV Security Code</label>
                    <input 
                      type="password" 
                      required 
                      placeholder="123" 
          ...
          <div className="flex items-center gap-3 bg-stone-50 border border-stone-200 p-4 rounded-2xl">
            ...
            <p className="text-[11px] text-stone-500 font-medium">
              Your transaction is protected with 256-bit SSL encryption. Local Atlas guides are verified and certified by the regional tourism office.
            </p>
          </div>
        ...
        <div className="bg-emerald-50/70 border-2 border-emerald-500/30 rounded-3xl p-6 space-y-5 h-fit shadow-md">
          ...
          <div className="space-y-3 text-xs">
            <div>
              <span className="font-bold text-stone-900 block text-sm">{bookingDetails.title}</span>
              ...
              <div className="flex items-center gap-2 text-stone-500 mt-0.5">
                ...
                <span>{bookingDetails.duration} ({bookingDetails.startDate})</span>
              </div>
            </div>

            <div className="border-t border-emerald-200/80 pt-3 space-y-2 font-medium text-stone-600">
              <div className="flex justify-between">
                <span>Certified Guide Fee:</span>
                <span className="font-bold text-stone-900">{bookingDetails.guideFee} MAD</span>
              </div>
              <div className="flex justify-between">
                <span>Equipment Rental Subtotal:</span>
                <span className="font-bold text-stone-900">{bookingDetails.gearTotal} MAD</span>
              </div>
              <div className="flex justify-between">
                <span>Safety & Gear Insurance:</span>
                <span className="font-bold text-stone-900">{bookingDetails.insuranceFee} MAD</span>
              </div>
            </div>
          </div>
