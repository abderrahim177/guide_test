...
export default function CheckoutPage() {
  ...
  const bookingDetails = {
    title: guideData.role ? `${guideData.role} Experience` : "High Atlas Trekking Experience",
    duration: `${bookingDetail.startDate || ''} to ${bookingDetail.endDate || ''}`,
    location: guideData.location || "Azilal Region",
    startDate: bookingDetail.startDate || new Date().toISOString().split("T")[0],
    guideFee: totalAmount,
    guideName: guideData.name || "Local Guide",
  };
    ...
    return (
      <div className="max-w-md mx-auto mt-12 bg-white border border-emerald-200 rounded-3xl p-8 text-center space-y-4 shadow-lg font-['Poppins',sans-serif]">
        ...
        <p className="text-xs text-stone-500">
          Your booking with {bookingDetails.guideName} has been confirmed. We sent confirmation details to your WhatsApp and email.
        </p>
        <button 
          onClick={() => navigate("/")}
        ...
        <form onSubmit={handleSubmit} className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                ...
                <div>
                  <span className="font-bold text-xs text-stone-900 block">Pay on Arrival</span>
                  <span className="text-[10px] text-stone-500">Pay cash directly to {bookingDetails.guideName}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 bg-stone-50 border border-stone-200 p-4 rounded-2xl">
            ...
            <p className="text-[11px] text-stone-500 font-medium">
              Your transaction is protected with 256-bit SSL encryption. Local Atlas guides are verified and certified.
            </p>
          </div>
          ...
          <div className="space-y-3 text-xs">
            <div>
              ...
              <div className="text-stone-600 mt-2 font-medium">
                Guide: <strong className="text-stone-900">{bookingDetails.guideName}</strong>
              </div>
            </div>
            ...
            <div className="border-t border-emerald-200/80 pt-3 space-y-2 font-medium text-stone-600">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-stone-900">{totalAmount} MAD</span>
              </div>
            </div>
          </div>
