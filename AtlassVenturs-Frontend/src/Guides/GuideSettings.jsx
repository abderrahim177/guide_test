...
export default function GuideSettings() {
  ...
  const handelSubmit = async (e) => {
    e.preventDefault();
    if (!newCert.title || !newCert.year) {
      setError("please insert required filds !");
      return;
    }
    ...
    try {
      const response = await axios.post(
        "http://127.0.0.1:8000/api/AddCertificat",
        newCert,
        {
          headers: {
  ...
  const handelFetchData = async() => {
    try{
      const token = localStorage.getItem('token')
      const response = await axios.get('http://127.0.0.1:8000/api/GetAllCertificates' , 
        {
          headers : {
            Authorization : `Bearer ${token}`,
            Accept : "application/json",
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-xs">
            Manage your guide profile, certifications, and system preferences.
          </p>
        </div>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
              ...
              <h2 className="font-bold text-slate-900 text-sm">
                Official Certifications & Degrees
              </h2>
            </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-slate-500 mt-0.5">
                        ...
                        <span className="truncate max-w-[150px]">
                          {cert.fileName || "certification.pdf"}
                        </span>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              <p className="text-[11px] text-slate-500">
                Select your preferred language for the guide dashboard and
          ...
          <div className="bg-slate-900 text-white p-5 rounded-2xl space-y-2">
            ...
            <p className="text-[11px] text-slate-300 leading-relaxed">
              Uploading updated official certificates speeds up your profile
              verification badge and increases visitor trust on AtlasVenture.
            </p>
          </div>
        </div>
      </div>
